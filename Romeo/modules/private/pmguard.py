...
from Romeo.database.pmpermitdb import get_approved_users, pm_guard
import Romeo.database.pmpermitdb as Romeo
...
async def setpmmsg(client, message):
    ...
    if not arg:
        ...
        return
    if arg == "default":
        await Romeo.set_block_message(Romeo.BLOCKED)
        await message.edit("**Block message set to default**.")
        return
    await Romeo.set_block_message(f"`{arg}`")
...
async def allow(client, message):
    ...
    pmpermit, pm_message, limit, block_message = await Romeo.get_pm_settings()
    await Romeo.allow_user(chat_id)
